...
import { SUBS_NAV_LINKS} from "@collections/subscriptions-links.collections";

export class SubNavLink {
...
@Component({
  selector: 'app-subscription-navbar',
  templateUrl: './subscription-navbar.component.html',
  styleUrls: ['./subscription-navbar.component.scss']
})
export class SubscriptionNavbarComponent implements OnChanges {


  @Input('appRoute') appRoute: string;

  public shownNavLinks: SubNavLink[] = [];
    public untouchedNavLinks: SubNavLink[] = [];
    public user = '';
  ...
  private setShownNavLinks(): void {
    ...
    SUBS_NAV_LINKS.forEach((navLink) => {
