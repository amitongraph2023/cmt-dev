...
@Service
@Slf4j
public abstract class BasePanaraNotificationGatewayService {
    @Value("${png.auth}")
    private String auth;
    ...
    protected <T> ResponseHolder<T> doDelete(Class<T> clazz, StopWatch stopWatch, Audit audit, EpsEndpoints stub, Object... params) {
        return doSendResponse(clazz, stopWatch, audit, HttpMethod.DELETE, null, stub, params);
    }

    protected <T> T doGet(Class<T> clazz, StopWatch stopWatch, EpsEndpoints stub, Object... params) {
        return doSend(clazz, stopWatch, HttpMethod.GET, null, stub, params);
    }

    protected <T> ResponseHolder<T> doGetResponse(Class<T> clazz, StopWatch stopWatch, EpsEndpoints stub, Object... params) {
        return doSendResponse(clazz, stopWatch, null, HttpMethod.GET, null, stub, params);
    }

    protected <T> ResponseHolder<T> doPost(Class<T> clazz, StopWatch stopWatch, Audit audit, Object body, EpsEndpoints stub, Object... params) {
        return doSendResponse(clazz, stopWatch, audit, HttpMethod.POST, body, stub, params);
    }

    protected <T> ResponseHolder<T> doPut(Class<T> clazz, StopWatch stopWatch, Audit audit, Object body, EpsEndpoints stub, Object... params) {
        return doSendResponse(clazz, stopWatch, audit, HttpMethod.PUT, body, stub, params);
    }

    private <T> ResponseHolder<T> doSendResponse(Class<T> clazz, StopWatch stopWatch, Audit audit, HttpMethod method, Object body, EpsEndpoints stub, Object... params) {
        ...
        if (response != null) {
            ...
            stopWatch.checkPoint("Error connecting with enterprise prospect service");
            return null;
        }
    }
    private <T> T doSend(Class<T> clazz, StopWatch stopWatch, HttpMethod method, Object body, EpsEndpoints stub, Object... params) {
        ProxyUserDTO proxyUserDTO = ProxyUserDTO.fromAuthenticatedUser(AuthenticatedUserManager.getAuthenticatedUser());
        WebClient client = WebClient
                .builder()
                .baseUrl(baseUrl)
